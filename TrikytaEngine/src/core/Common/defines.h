...
#define FREE(p)					if (p !=NULL ) {do { delete (p); p = NULL;} while(0);}
...
#define RemoveEventHandler(__event__, ...) EventManager::GetEventManager()->removeEventHandler<Events::__event__>(__VA_ARGS__);
#define ClearAllEventHandlers(__event__, ...) EventManager::GetEventManager()->clearAllEventHandlers<Events::__event__>(##__VA_ARGS__);
