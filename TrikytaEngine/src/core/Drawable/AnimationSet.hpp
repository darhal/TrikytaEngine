...
protected:
	AnimationSet(const std::string& p_Filename, Vec2<int> p_Size, Vec2<int> p_Position, int p_FramesMax, int p_FirstFrame) :
		Sprite(p_Filename, p_Size, p_Position), m_MaxFrames(p_FramesMax), m_CurrentAnimFrameID(p_FirstFrame)
	{
		init();
	}
	AnimationSet(const std::string& p_SpritesFilename, const std::string& p_ParserFilename, Vec2<int> p_Size, Vec2<int> p_Position, float p_RequiredTimeToUpdate) :
...
//	inline std::vector<SDL_Rect>* getSpriteFramesCoords() { return SpriteFramesCoords; }
	...
	static AnimationSet* Create(const std::string& p_SpritesFilename, const std::string& p_ParserFilename, Vec2<int> p_Size, Vec2<int> p_Position, float p_RequiredTimeToUpdate = 1 / 60)
	{
